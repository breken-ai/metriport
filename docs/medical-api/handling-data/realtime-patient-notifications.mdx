...
Each real-time notification we send via webhook has a payload that includes key information about the event, and a url field to a [Patient Encounter Bundle](/medical-api/handling-data/patient-encounter-bundle) containing robust clinical data for the encounter.
...
If you want to access the clinical data, you need only download the [Patient Encounter Bundle](/medical-api/handling-data/patient-encounter-bundle) from the url. To work with the data, we recommend you understand [The Encounter Model](#the-encounter-model) below.
...
To provide comprehensive data about the Encounter, we serve a [Patient Encounter Bundle](/medical-api/handling-data/patient-encounter-bundle) (a FHIR [Bundle](https://hl7.org/fhir/R4/bundle.html)) via the `url` in the patient notification.
...
The first two items in every bundle for a Metriport's Patient Notification are the applicable patient & root Encounter. Every item thereafter is a resource referenced by the Encounter.
...
To enable real-time notifications for:
  - ADTs (Admission, Discharge, Transfer): Networks that transmit live patient status updates as they move through healthcare facilities.  
...
Use this feature by [setting it up in one or more Cohorts](/medical-api/handling-data/cohorts) and associating the patients you want to have notifications enabled.
...
The Encounter is the core resource in a [Patient Encounter Bundle](/medical-api/handling-data/patient-encounter-bundle). Below is an example of a FHIR Encounter resource - one piece of the data you'll receive when downloading from the URL you receive in the notification.
