 ...
 * - always create consolidated bundles per patient and store them in the consolidated folder;
 * - if `--use-fhir-server` is set, it will:
 *   - remove all partitions from the FHIR server (if `--cleanup` is also set);
 *   - create a tenant on the FHIR server;
 *   - load the consolidated bundles into the FHIR server;
...
const consolidatedTotalResourceCountStatsLocation = `${consolidatedFolderName}/total-resource-counts.json`;
const consolidatedResourceCountPerPatientStatsLocation = `${consolidatedFolderName}/resource-counts-per-patient.json`;
...
export async function main() {
  ...
  const { nonXMLBodyCount, totalAttachments } = await convertCDAsToFHIR(
    cdaLocation,
    relativeFileNames,
    parallelConversions,
    startedAt,
  ...
  console.log(`Final results:\n{
    "xmls": ${ccdaFileNames.length},
    "cdas": ${clinicalDocuments.length},
    "total": ${totalResourceCountStats.total},
    "totalAttachments": ${totalAttachments},
    "countPerType": ${JSON.stringify(totalResourceCountStats.countPerType, null, 2)}
  }`);

  // Always create consolidated bundles per patient
  ...
  await createConsolidatedBundles(filesByPatient);

  // Count resources across all consolidated bundles
  await countConsolidatedResources(filesByPatient);

  // Only do cleanup and insert if useFhirServer flag is set
  if (useFhirServer) {
    ...
    await uploadConsolidatedBundlesIntoFhirServer(filesByPatient);
  }
...
async function createConsolidatedBundles(filesByPatient: Record<string, string[]>) {
  console.log(
  ...
  for (const [patientId, fileNames] of Object.entries(filesByPatient)) {
    ...
    try {
      await createConsolidatedBundleForPatient(patientId, fileNames);
      console.log(`Successfully created consolidated bundle for patient ${patientId}`);
    } catch (error) {
      console.error(`Error creating consolidated bundle for patient ${patientId}:`, error);
      // intentionally not throwing, continue processing other patients
    }
  }

  console.log(`\n=== CONSOLIDATED BUNDLES CREATED ===`);
  console.log(`All ${Object.keys(filesByPatient).length} patients processed!`);
  console.log(`Consolidated bundles saved in: ${consolidatedFolderName}`);
}

async function countConsolidatedResources(filesByPatient: Record<string, string[]>) {
  console.log(`Counting resources across all consolidated bundles...`);

  // Count total resources across all consolidated bundles and per patient using existing method
  const totalCountPerType: Record<string, number> = {};
  let grandTotal = 0;
  const resourceCountsPerPatient: Array<{
    patientId: string;
    countPerType: Record<string, number>;
    total: number;
  }> = [];

  for (const patientId of Object.keys(filesByPatient)) {
    try {
      const consolidatedBundlePath = `${consolidatedFolderName}/${patientId}/consolidated.json`;
      const count = await getResourceCountByFile(consolidatedBundlePath);

      resourceCountsPerPatient.push({
        patientId,
        countPerType: count.countPerType,
        total: count.total,
      });

      // Aggregate totals
      for (const [resourceType, countValue] of Object.entries(count.countPerType)) {
        totalCountPerType[resourceType] = (totalCountPerType[resourceType] ?? 0) + countValue;
      }
      grandTotal += count.total;
    } catch (error) {
      console.error(`Error counting resources for patient ${patientId}:`, error);
      // intentionally not throwing, continue processing other patients
    }
  }

  // Save total counts across all consolidated bundles
  const totalStats = {
    total: grandTotal,
    countPerType: totalCountPerType,
  };
  storeStats(totalStats, consolidatedTotalResourceCountStatsLocation);
  console.log(`Consolidated resources: ${JSON.stringify(totalStats.countPerType, null, 2)}`);
  console.log(`Consolidated total: ${totalStats.total}`);
  console.log(`Final results:\n{
    "patients": ${Object.keys(filesByPatient).length},
    "total": ${totalStats.total},
    "countPerType": ${JSON.stringify(totalStats.countPerType, null, 2)}
  }`);

  // Save resource counts per patient
  const perPatientStats = {
    totalPatients: resourceCountsPerPatient.length,
    patients: resourceCountsPerPatient,
  };
  storeStats(perPatientStats, consolidatedResourceCountPerPatientStatsLocation);
  console.log(
    `Resource counts per patient saved to: ${consolidatedResourceCountPerPatientStatsLocation}`
  );
}

async function uploadConsolidatedBundlesIntoFhirServer(filesByPatient: Record<string, string[]>) {
  console.log(
    `Uploading consolidated bundles to FHIR server for ${
      Object.keys(filesByPatient).length
    } patients...`
  );

  for (const [patientId] of Object.entries(filesByPatient)) {
    console.log(`Uploading patient ${patientId}`);
    try {
      const consolidatedBundle = await loadConsolidatedBundleForPatient(patientId);
      await uploadConsolidatedIntoFhirServer(consolidatedBundle, patientId);
      console.log(`Successfully uploaded patient ${patientId}`);
    } catch (error) {
  ...
  if (errorFiles.length > 0) {
    ...
    console.log(`\n=== SUCCESS ===`);
    console.log(`All ${Object.keys(filesByPatient).length} patients uploaded successfully!`);
    console.log(`Consolidated bundles saved in: ${consolidatedFolderName}`);
  }
...
async function loadConsolidatedBundleForPatient(patientId: string): Promise<Bundle> {
  const consolidatedBundlePath = `${consolidatedFolderName}/${patientId}/consolidated.json`;
  const contents = getFileContents(consolidatedBundlePath);
  const bundle = parseFhirBundle(contents);
  if (!bundle) {
    throw new Error(`No valid consolidated bundle found for patient ${patientId}`);
  }
  return bundle;
}
