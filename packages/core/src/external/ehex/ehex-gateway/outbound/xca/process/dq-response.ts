...
export function parseDocumentReference({
  ...
  const slots = toArray(extrinsicObject?.Slot).filter(Boolean);
  const externalIdentifiers = toArray(extrinsicObject?.ExternalIdentifier).filter(Boolean);
  const classifications = toArray(extrinsicObject?.Classification).filter(Boolean);
