...
/**
 * Test an SFTP connection to Quest Diagnostics. Will only work if it is being run from a server
 * within the VPC corresponding to the environment you are testing (production or staging).
 *
 * Usage:
 * npm run quest -- sftp connect
 *
 * You can list all files on the remote SFTP server with:
 *
 * npm run quest -- sftp list /path/to/directory
 *
 * You can read a file from the remote SFTP server with:
 *
 * npm run quest -- sftp read /path/to/file
 *
 * You can write a file to the remote SFTP server with:
 *
 * npm run quest -- sftp write /path/to/local/file /path/to/remote/file
 *
 ...
 *
 * npm run quest -- sftp sync /path/to/remote/directory
 *
 * You can check if a file exists in the SFTP server with:
 *
 * npm run quest -- sftp exists /path/to/remote/file
 */
const sftpAction = new Command();

/**
 * Executes an SFTP action using the Quest Diagnostics SFTP client.
 *
