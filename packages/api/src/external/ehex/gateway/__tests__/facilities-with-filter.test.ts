...
import { makeEhexDirectoryEntry } from "../../__tests__/ehex-directory";

describe("facilitiesWithEpicFilter", () => {
  it("returns all facilities when epic is true", async () => {
    const isEpicEnabled = true;

    const epicFacilityEntry = makeEhexDirectoryEntry({
      rootOrganization: EPIC_ORG_NAME,
    });

    const nonEpicFacilityEntry = makeEhexDirectoryEntry({
  ...
  it("filters out all facilities when epic is enabled and facilities are all epic", async () => {
    ...
    const epicFacilityEntryTwo = makeEhexDirectoryEntry({
      rootOrganization: EPIC_ORG_NAME,
    });
