...
export async function initTermServer(): Promise<void> {
  const { log } = out("initTermServer");
  log("Initializing terminology server");
  const key = "terminology_v2.2.db";
  ...
  if (!existsSync(dbPath)) {
    ...
    log(`Downloading file from S3: ${bucket}/${key}`);
    const db = await s3Utils.downloadFile({ bucket, key });
    log(`Downloaded file size: ${db.length} bytes`);
    writeFileSync(dbPath, db);
  } else {
    log(`Database file already exists at ${dbPath}`);
  }
