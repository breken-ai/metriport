...
export function createSecurityHeader({
  ...
  const saml2NameID = `CN=ehex.metriport.com,OU=Healtheway,O=MetriportInc.,ST=California,C=US`;
...
export function extractPublicKeyInfo(certificatePem: string) {
  ...
  if (asn1.offset === -1) {
    throw new MetriportError("Error during ASN.1 parsing.");
  }

  const certificate = new pkijs.Certificate({ schema: asn1.result });
  const publicKey = certificate.subjectPublicKeyInfo;
  if (!publicKey.parsedKey) {
    throw new MetriportError("Public key information is undefined.");
  }
