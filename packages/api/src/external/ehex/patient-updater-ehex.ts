...
const maxNumberOfParallelRequestsToEhex = 10;

/**
 * Implementation of the PatientUpdater that executes the logic on Ehex.
 */
export class PatientUpdaterEhex extends PatientUpdater {
  public async updateAll(
    ...
    async function updatePatient(patient: Patient) {
      try {
        ...
        failedUpdateCount++;
        const msg = `Failed to update Ehex patient`;
    ...
    await executeAsynchronously(patients, async patient => updatePatient(patient), {
      numberOfParallelExecutions: maxNumberOfParallelRequestsToEhex,
    });
