...
AI summaries are available in the data warehouse under the [`Binary`](/medical-api/handling-data/schema/binary) resource table. The AI brief text is stored as Base64-encoded data in the `data` column, and can be identified by the `meta_source` field which contains `"metriport:ai-generated-brief"`.

<Info>
  The `data` field contains Base64-encoded text. You'll need to decode it to retrieve the AI brief content.
</Info>
