...
const folderName = buildPathInsideRunsFolder(`1-fhir-to-csv`);
...
async function main({
  ...
  if (checkConsolidatedExists) {
    ...
    if (patientsWithoutConsolidatedData.length > 0) {
      ...
      const fileName = `${folderName}/${fhirToCsvJobId}/patients-without-consolidated-data.txt`;
      initFile(fileName);
  ...
  const failedPatientIds = await fhirToCsvHandler.processFhirToCsvBulk({
    ...
    progressCallback: (current: number, total: number) => {
      if (current % 100 === 0) log(`>>> Sent ${current} patients to queue (${total} patients)`);
    },
  });
  log(``);
...
async function displayWarningAndConfirmation(
  ...
  const patientsWithoutConsolidatedDataMsg =
    patientsWithoutConsolidatedData.length > 0
      ...
      : "";
  const msg =
    `You are about to trigger the conversion of ${patientsToInsert.length} patients (out of ${totalPatients}) of ` +
