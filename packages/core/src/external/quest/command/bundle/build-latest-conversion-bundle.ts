...
/**
 * Deduplicates and merges all Quest lab conversion bundles for a given patient.
 * @param cxId - The ID of the care experience.
 * @param patientId - The ID of the patient.
 * @returns A latest conversion bundle with all Quest lab data, or undefined if no bundles were found.
 */
export async function buildLatestConversionLabBundle({
  cxId,
  patientId,
}: {
  cxId: string;
  ...
  const { log } = out("quest.build-latest-conversion");
  ...
  const bundles = await getAllLabConversionBundles({ s3, cxId, patientId, labBucketName, log });
  ...
  const latestBundleName = buildPatientLatestLabConversionFileName({ cxId, patientId });
 ...
 * @returns
 */
async function getAllLabConversionBundles({
  s3,
  cxId,
  patientId,
  labBucketName,
  ...
  for (const file of files) {
    ...
    if (file.Key.includes(buildPatientLatestLabConversionFileName({ cxId, patientId }))) {
      continue;
    }
    const fileContents = await executeWithRetriesS3(async () =>
      s3.getFileContentsAsString(labBucketName, file.Key as string)
    );
