...
export function buildPathInsideRunsFolder(folder?: string): string {
...
export function getPathNameForOrg(orgName: string): string {
  return orgName.replace(/[,.]/g, "").replace(/_/g, "-");
}
...
export function getFileNameForOrgWithTimestamp({
  ...
  const pathName = orgName ? getPathNameForOrg(orgName) : "";
  return `${pathName}${timestampAsStr}${extensionAsStr}`;
}
