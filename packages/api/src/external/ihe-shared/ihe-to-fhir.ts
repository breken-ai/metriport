...
export function iheToFhirDocumentReference({
  docId,
  docRef,
  docStatus,
  patientId,
  ...
  patientId: string;
  source: MedicalDataSource.CAREQUALITY | MedicalDataSource.EHEX;
  contentExtension: MetriportDataSourceExtension;
  orgName?: string;
}): DocumentReferenceWithId {
  const sourceExtension = source === MedicalDataSource.EHEX ? ehexExtension : cqExtension;
  ...
  const organizationFromIheDirectory = mapToContainedOrganization(orgName);
  if (organizationFromIheDirectory) contained.push(organizationFromIheDirectory);
  ...
  const updatedDocRef: DocumentReferenceWithId = {
    ...
    ...(docRef.confidentialityCoding
      ...
      : {}),
    content: generateIheFhirContent(baseAttachment, contentExtension, docRef.url),
...
function generateIheFhirContent(
  baseAttachment: {
    contentType?: string;
    size?: number;
    creation?: string;
  ...
  const iheFhirContent = createDocReferenceContent({
    ...baseAttachment,
    location: location,
    extension: [contentExtension],
  });

  return [iheFhirContent];
}
...
export function dedupeContainedResources(combined: Resource[]): Resource[] {
  const seen = new Set();
  const deduped = combined.filter(resource => {
    const resourceStr = JSON.stringify(resource, Object.keys(resource).sort());
