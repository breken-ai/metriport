...
select
        s.id as medication_administration_id
    ,   t.property
...
from {{ref('stage__medicationadministration')}} s
...
where t.reference_value is not null
    ...
    and (
        s.medicationreference_reference is not null or s.subject_reference is not null or
