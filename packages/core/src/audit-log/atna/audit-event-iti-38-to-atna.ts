...
/**
 * Converts a FHIR AuditEvent for ITI-38 (Document Query) to an ATNA AuditMessage.
 *
 ...
 *
 * For Query Entity (ITI-38 specific):
 *   - query: Base64-encoded query XML (OPTIONAL - not required for ITI-38)
 *   - detail: Optional detail elements
 */
export function auditEventIti38ToAtna(auditEvent: AuditEvent): AtnaAuditEvent {
  ...
  activeParticipants.push(buildCombinedSourceActiveParticipant(gatewayAgent, isOutbound));

  // Process Destination agents
  for (const agent of destinationAgents) {
    activeParticipants.push(buildActiveParticipant(agent, "destination"));
  }
  ...
  if (auditEvent.entity && auditEvent.entity.length > 0) {
    const entityObjects = auditEvent.entity
      .filter(entity => {
        // Filter out report entities (OperationOutcome, etc.)
      ...
      .map(entity => buildParticipantObject(entity, eventTypeCode, auditEvent))
...
function buildCombinedSourceActiveParticipant(
  ...
  const activeParticipant: Record<string, unknown> = {
    "@_UserID": userId,
    "@_UserIsRequestor": isOutbound || gatewayAgent.requestor ? "true" : "false",
 ...
 */
function buildActiveParticipant(
...
/**
 * Builds a ParticipantObjectIdentification element for ITI-38.
 * ITI-38 query entities do NOT require the 'query' field or 'detail' elements.
 */
function buildParticipantObject(
  ...
  const isQueryEntity = typeCode === "2" && roleCode === "24";
  ...
  if (isQueryEntity) {
    ...
    if (!queryId) {
      throw new MetriportError(
        "ITI-38 query entity requires 'what.identifier.value' or 'what.display'"
      );
    }
    participantObject["@_ParticipantObjectID"] = queryId;
    ...
    // ParticipantObjectQuery is MANDATORY for ITI-38
    if (!entity.query) {
      throw new MetriportError(
        "ITI-38 Audit Event missing mandatory 'entity.query' (Base64 AdhocQueryRequest)"
      );
    }
    participantObject.ParticipantObjectQuery = entity.query;

    // Build ParticipantObjectDetail array
    const details: Array<{ "@_type": string; "@_value"?: string }> = [];

    // QueryEncoding specifies the character encoding of the query (defaults to UTF-8)
    const queryEncoding = "UTF-8";
    details.push({
      "@_type": "QueryEncoding",
      "@_value": Buffer.from(queryEncoding, "utf-8").toString("base64"),
    });

    // Add other details from entity
    if (entity.detail && entity.detail.length > 0) {
      for (const detail of entity.detail) {
        // Use the full URN for homeCommunityId per IHE ITI TF-2b spec for XCA
        let detailType = detail.type;
        if (detailType?.toLowerCase().includes("homecommunityid")) {
          detailType = "urn:ihe:iti:xca:2010:homeCommunityId";
        }
