...
export type GetEnrolledPatientsForBackfillRequest = {
  cxId: string;
  rosterId: string;
};

export async function getEnrolledPatientsForBackfill({
  cxId,
  rosterId,
}: GetEnrolledPatientsForBackfillRequest): Promise<Patient[]> {
  const { log } = out(`Quest getEnrolledPatientsForBackfill - cxId ${cxId}, rosterId ${rosterId}`);
  const api = axios.create();
  const enrolledPatients: Patient[] = [];
  const questRosterRoute = `${questRosterBaseRoute}/${QuestRosterType.BACKFILL}?cxId=${cxId}&rosterId=${rosterId}`;
