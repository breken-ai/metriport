...
/**
 * This script triggers network queries for pharmacy sources for multiple patients.
 * It makes parallel requests to the API to query pharmacy documents for each patient.
 *
 ...
 *
 * Update the `patientIds` array with the list of Patient IDs you want to query pharmacy sources for.
 * Alternatively, you can provide a file with patient IDs, one per line.
 * If no patient IDs are provided and no file is specified, ALL patient IDs for the customer will
 * be fetched from the API.
 *
 * Successfully processed patient IDs are saved in a file in the `runs/backfill-roster-surescripts` folder,
 * named with the customer's name and timestamp, e.g.,:
 * $ packages/utils/runs/backfill-roster-surescripts  /<cx-name>_2025-06-19T06:56:19.714Z.success.patientIds.txt
 *
 * Any errors encountered during processing are saved in two files in the `runs/backfill-roster-surescripts` folder,
 * named with the customer's name and timestamp, e.g.,:
 * $ packages/utils/runs/backfill-roster-surescripts/<cx-name>_2025-06-19T06:56:19.714Z.error.patientIds.txt
 * $ packages/utils/runs/backfill-roster-surescripts/<cx-name>_2025-06-19T06:56:19.714Z.error.txt (detailed error)
 *
 ...
 *
 * Execute this with:
 * $ ts-node src/surescripts/bulk-add-patients-to-backfill-roster.ts
 */
...
// output stuff
const getOutputFileName = buildGetDirPathInside(`backfill-roster-surescripts`);
const patientsWithErrors: string[] = [];

const program = new Command();
program
  .name("bulk-add-patients-to-backfill-roster")
  .description("CLI to trigger pharmacy network queries for multiple patients.")
  .showHelpAfterError();
...
async function displayWarningAndConfirmation(
  ...
  const msg = `You are about to trigger pharmacy network queries for ${scope} patients of the org/cx ${orgName}.`;
  log(msg);
