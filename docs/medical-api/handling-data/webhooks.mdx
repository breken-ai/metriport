---
title: "Receiving Webhooks (Legacy)"
icon: "paper-plane"
description: "Receive status updates as soon as data becomes available."
---

<Warning>
  **Legacy Webhook Flow:** This page documents webhooks for the legacy [Document Query](/medical-api/legacy-api-reference/document/start-document-query) flow.
  
  We recommend migrating to the [Network Query](/medical-api/api-reference/network/start-network-query) flow, which provides a unified way to query all health data networks (HIEs, pharmacies, and laboratories) with improved webhook events. See the [updated webhooks guide](/medical-api/handling-data/webhooks-with-nq) for details.
</Warning>
...
<ResponseField name="patients" type="PatientConsolidatedData[]" required>
  ...
  <Expandable title="PatientConsolidatedData properties">
    ...
    <ResponseField name="bundle" type="Bundle">
      ...
      <ul>
        <li>Main attachment with the requested format (JSON, HTML, or PDF)</li>
        <li>Gzip-compressed attachment (JSON only) with `contentType: "application/gzip"` for faster downloads</li>
      </ul>
