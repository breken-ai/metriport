...
export async function getNonExistentDocRefs(
  ...
  const { log } = out(`Ehex getNonExistentDocRefs, cx: ${cxId}, pt: ${patientId}`);
...
async function checkDocRefsExistInS3(
  ...
  const { log } = out(`Ehex checkDocRefsExistInS3 - patient ${patientId}`);
  ...
  await executeAsynchronously(
    ...
    async doc => {
      try {
        ...
        capture.message(msg, {
          extra: {
            context: `ehex.checkDocRefsExistInS3`,
...
function isEligibleForRedownload(creation: string | null | undefined): boolean {
  ...
  if (!Config.isProdEnv()) return true;
