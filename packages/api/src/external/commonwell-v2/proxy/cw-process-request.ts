...
import { isCommonwellLogsEnabled } from "@metriport/core/command/feature-flags/domain-ffs";
import {
  docContributionFileParam,
  getDocContributionURL,
} from "@metriport/core/external/commonwell-v1/document/document-contribution";
import {
  buildBundleEntry,
...
import { parseMetadataXmlToDocumentReference } from "@metriport/core/shareback/metadata/parse-metadata-xml";
...
export async function processRequest(
  ...
  for (const fileContents of metadataFileContents) {
    try {
      const additionalDocRef = await parseMetadataXmlToDocumentReference({
        patientId,
      ...
      if (swallowErrors) {
        log(`Error parsing metadata XML during inbound request: ${errorToString(err)}`);
        continue;
      }

      // TODO ENG-1664 Remove this once errors get to zero per day
      if (shouldRetryMetadataCleanup(err)) throw err;
  ...
  if (await isCommonwellLogsEnabled()) log(`Response: ${JSON.stringify(bundle)}`);

  return bundle;
}

// TODO ENG-1664 Remove this once errors get to zero per day
export function shouldRetryMetadataCleanup(error: unknown): boolean {
  return String(error).includes("Document ID mapping not found");
}

export async function getPatientAndCxFromRequest(
  req: Request
